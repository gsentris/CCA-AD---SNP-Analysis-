...
X = data[['growth', 'profit', 'new']]
y = data[['create', 'mech', 'abs', 'math']]  # Shape: (442, 4)
...
X_columns = ["Sales Growth", "sales profitability", "New Account Sales"]

Y_columns = ["Creativity","Mechanical Reasoning","Abstract Reasoning","Mathematics"]
...
Y_scaled = scaler_Y.fit_transform(y)  # Fit and transform Y separately
...
# Plot for X loadings
plt.subplot(1, 2, 1)
plt.barh(np.arange(X.shape[1]), cca.x_loadings_[:, 0], align='center')
...
# Plot for Y loadings
plt.subplot(1, 2, 2)
plt.barh(np.arange(Y_scaled.shape[1]), cca.y_loadings_[:, 0], align='center')
